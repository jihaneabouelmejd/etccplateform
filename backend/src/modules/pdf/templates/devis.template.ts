...
import type { PDFLanguage } from '../pdf.service';
import type { DevisPDFData } from '../pdf.service';
...
interface CompanyData {
  ...
  website?: string|null; patente?: string|null;
}
interface DevisTemplateInput extends DevisPDFData { company: CompanyData; lang: PDFLanguage; }

const CSS = `
*{box-sizing:border-box;margin:0;padding:0;}
body{
  font-family:Arial,Helvetica,sans-serif;
  font-size:13px;color:#0C0C0C;background:#FAFAFA;
...
.brand-name{
  ...
  color:#0C0C0C;text-transform:uppercase;line-height:1;
}
.doc-id{position:relative;z-index:1;text-align:right;padding-top:6px;}
.doc-type{
  font-size:8.5px;font-weight:700;letter-spacing:4.5px;
  text-transform:uppercase;color:#0C0C0C;display:block;margin-bottom:3px;
}
.doc-num{
  font-size:32px;font-weight:900;color:#0C0C0C;
...
.bar{
  height:3px;
  background:linear-gradient(90deg,#F5C842 0%,#D4A017 65%,transparent 100%);
  margin:22px 48px 0;
}

/* META */
.meta{
  display:flex;margin:0 48px;padding:18px 0;
...
.ml{
  ...
  text-transform:uppercase;color:#D4A017;margin-bottom:3px;display:block;
}
...
.ptag{
  ...
  color:#ADADAD;margin-bottom:7px;display:block;
}
...
/* OBJET */
.objet{margin:0 48px 16px;display:flex;align-items:center;gap:12px;}
.objet-badge{
  background:#F5C842;color:#0C0C0C;font-size:7.5px;font-weight:700;
  letter-spacing:2.5px;text-transform:uppercase;padding:4px 10px;
  border-radius:2px;white-space:nowrap;
}
.objet-text{font-size:13px;font-weight:500;color:#1A1A1A;font-style:italic;}
...
.twrap thead th{
  ...
  letter-spacing:2px;text-transform:uppercase;color:#0C0C0C;text-align:left;
}
...
.tdsub{font-size:10.5px;color:#6B6B6B;font-weight:300;line-height:1.5;}

/* BOTTOM */
.bottom{margin:0 48px 20px;display:flex;justify-content:space-between;align-items:flex-start;gap:24px;}
.notes{flex:1;}
.notes h4{font-size:8px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:#D4A017;margin-bottom:8px;}
.notes p{font-size:11px;color:#6B6B6B;line-height:1.8;max-width:240px;}
...
.tline .tl{color:#6B6B6B;font-weight:400;}
.tline .tv{font-weight:500;color:#1A1A1A;}
.tline.disc .tl{font-size:10.5px;color:#DC2626;}
.tline.disc .tv{color:#DC2626;}
.tfinal{
  background:#F5C842;border-radius:5px;padding:14px 16px;
...
/* CONDITIONS */
.conds{
  margin:auto 48px 20px;background:#F4F4F4;border-radius:5px;
  padding:15px 18px;display:grid;grid-template-columns:1fr 1fr;gap:18px;
}
...
/* SIGNATURES */
.sigs{margin:0 48px 32px;}
.sblock h5{font-size:8px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:#6B6B6B;margin-bottom:10px;}
.sarea{border:1.5px dashed #E6E6E6;border-radius:5px;height:90px;display:flex;align-items:center;justify-content:center;padding:8px 12px;width:260px;}
.sarea img{max-height:70px;max-width:200px;object-fit:contain;display:block;margin:0 auto;}

/* FOOTER */
.footer{
  margin-top:auto;background:#F5C842;
...
export function devisTemplate(data: DevisTemplateInput): string {
  ...
  const signatureBlock = stampSrc
    ? `<img src="${stampSrc}" style="max-height:70px;max-width:200px;object-fit:contain;display:block;margin:0 auto;"/>`
...
<div class="meta">
  ...
  <div class="mc">
    <span class="ml">Valide jusqu'au</span>
...
<div class="bottom">
  <div class="notes">
    ${data.notes ? `<h4>Notes</h4><p>${data.notes}</p>` : ''}
    ${data.payment_terms ? `<h4 style="margin-top:10px">Conditions de paiement</h4><p>${data.payment_terms}</p>` : ''}
  </div>
  <div class="totals">
    ${data.discount_rate > 0 ? `
    <div class="tline">
      <span class="tl">Total HT brut</span>
      <span class="tv">${f(data.total_ht_brut)} DH</span>
    </div>
    <div class="tline disc">
      <span class="tl">Remise (${f(data.discount_rate)} %)</span>
      <span class="tv">− ${f(data.discount_amount)} DH</span>
    </div>` : ''}
    <div class="tline">
      <span class="tl">Total HT</span>
      <span class="tv">${f(data.total_ht_net)} DH</span>
    </div>
    <div class="tline">
      <span class="tl">TVA ${f(data.tva_rate)} %</span>
      <span class="tv">${f(data.tva_amount)} DH</span>
    </div>
    <div class="tfinal">
      <span class="tfl">Total TTC</span>
...
<div class="conds">
  <div class="cond">
    <h5>Validité du devis</h5>
    <p>Ce devis est valable <strong>${data.validity_days} jours</strong> à compter de sa date d'émission.</p>
  </div>
  <div class="cond">
    <h5>Conditions de paiement</h5>
    <p>${data.payment_terms || 'Modalités à convenir entre les parties.'}</p>
  </div>
</div>

<div class="sigs">
  <div class="sblock">
    <h5>Bon pour accord — ETCC</h5>
    <div class="sarea">${signatureBlock}</div>
  </div>
</div>
