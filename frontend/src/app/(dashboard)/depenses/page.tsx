...
import { Plus, Check, X, Camera, Pencil, Trash2, FileText, Download, ExternalLink, Users, AlertCircle, ChevronDown, ChevronRight, History } from 'lucide-react';
...
export default function DepensesPage() {
  ...
  const catLabel: Record<string, string> = {
    MATERIEL:       t('dep.materiel'),
    MAIN_OEUVRE:    t('dep.main_oeuvre'),
    TRANSPORT:      t('dep.transport'),
    CARBURANT:      t('dep.carburant'),
    OUTILS:         t('dep.outils'),
    SOUS_TRAITANCE: t('dep.sous_trait'),
    AUTRE:          t('dep.autre'),
  };
  ...
  const [form, setForm] = useState({ description:'', amount:'', category:'MATERIEL', payment_method:'ESPECES', project_id:'', date:new Date().toISOString().slice(0,10), notes:'', receipt_url:'' });
  ...
  const [detteForm, setDetteForm] = useState({ nom:'', description:'', montant:'', date:new Date().toISOString().slice(0,10), project_id:'', notes:'' });
  ...
  const openCreate = () => {
    setEditTarget(null);
    setForm({ description:'', amount:'', category:'MATERIEL', payment_method:'ESPECES', project_id:'', date:new Date().toISOString().slice(0,10), notes:'', receipt_url:'' });
    setReceiptPreview(''); setReceiptIsPdf(false); setSaveError(''); setShowForm(true);
  };

  const openEdit = (d: any) => {
    setEditTarget(d);
    setForm({ description:d.description, amount:String(d.amount), category:d.category, payment_method:d.payment_method||'ESPECES', project_id:d.project?.id||'', date:d.date?d.date.slice(0,10):new Date().toISOString().slice(0,10), notes:d.notes||'', receipt_url:d.receipt_url||'' });
    setReceiptPreview(''); setReceiptIsPdf(false); setSaveError(''); setShowForm(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const payload = { description:form.description.trim(), amount:parsedAmount, category:form.category, payment_method:form.payment_method||'ESPECES', project_id:form.project_id||undefined, date:form.date?new Date(form.date):undefined, notes:form.notes||undefined, receipt_url:form.receipt_url||undefined };
    try {
  ...
  // Group by worker
  const moByWorker = moList.reduce((acc: Record<string,any>, d) => {
    const name = d.submitter ? `${d.submitter.first_name||''} ${d.submitter.last_name||''}`.trim() : 'Inconnu';
    if (!acc[name]) acc[name] = { name, items:[], total:0, paye:0, pending:0 };
    acc[name].items.push(d);
    acc[name].total += Number(d.amount);
    if (d.status === 'APPROVED') acc[name].paye += Number(d.amount);
    if (d.status === 'PENDING')  acc[name].pending += Number(d.amount);
    return acc;
  }, {});
  const moWorkers = Object.values(moByWorker).sort((a:any,b:any) => b.total - a.total);
  ...
  const openCreateDette = () => {
    setEditDette(null);
    setDetteForm({ nom:'', description:'', montant:'', date:new Date().toISOString().slice(0,10), project_id:'', notes:'' });
    setDetteError(''); setShowDetteForm(true);
  };

  const openEditDette = (d: any) => {
    setEditDette(d);
    setDetteForm({ nom:d.nom, description:d.description, montant:String(d.montant), date:d.date?d.date.slice(0,10):new Date().toISOString().slice(0,10), project_id:d.project_id||'', notes:d.notes||'' });
    setDetteError(''); setShowDetteForm(true);
  };
  ...
  const handleSaveDette = async (e: React.FormEvent) => {
    ...
    setSavingDette(true);
    const payload = { nom:detteForm.nom.trim(), description:detteForm.description||detteForm.nom, montant:Number(detteForm.montant), date:detteForm.date||undefined, project_id:detteForm.project_id||undefined, notes:detteForm.notes||undefined };
    try {
  ...
  const handlePayDette = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSelectedDette(r.data);
      setDettes(prev => prev.map(d => d.id === r.data.id ? r.data : d));
      await dettesApi.stats().then(rs => setDetteStats(rs.data));
      setShowPayDetteForm(false);
      setPayDetteForm({ montant:'', mode:'ESPECES', date:new Date().toISOString().slice(0,10), notes:'' });
    ...
    <div>
      ...
      <div className="flex justify-between items-start mb-5">
        <div>
          ...
          <p className="text-sm text-honey-caramel mt-0.5">{isMgr ? 'Gestion des depenses, main d\'oeuvre et dettes' : 'Mes depenses soumises'}</p>
        </div>
        <div className="flex gap-2">
          ...
          {tab === 'depenses' && <button onClick={openCreate} className="btn-primary text-sm"><Plus size={13}/> Soumettre depense</button>}
            ...
            <div className="bg-gradient-to-br from-honey-beige-soft to-honey-cream border border-honey-beige-soft rounded-lg p-4 mb-4">
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  ...
                  { label:'Approuvees',  value:formatCurrency(stats?.approved_amount||0), color:'text-green-600' },
                  { label:'Refusees',    value:stats?.rejected_count||0,                  color:'text-red-500' },
                ].map(kpi => (
          ...
          <div>
            ...
            {detteStats && (
              <div className="grid grid-cols-3 gap-3 mb-4">
                {[
                  ...
                  { label:'Total paye',    value:formatCurrency(detteStats.paye||0),   color:'#10B981' },
                  { label:'Reste a payer', value:formatCurrency(detteStats.reste||0),  color:'#EF4444' },
                ].map(k => (
                  <div key={k.label} style={{ ...card, textAlign:'center' }}>
            ...
            : dettes.length===0 ? (
              <div style={{ ...card, textAlign:'center', padding:'48px 20px' }}>
                ...
                <p style={{ fontSize:14, fontWeight:600, color:'#8E5915' }}>Aucune dette enregistree</p>
                <p style={{ fontSize:12, color:'#B8A090', marginTop:4 }}>Ajoutez les personnes à qui vous devez de l'argent</p>
              </div>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {dettes.map((d:any) => {
                  ...
                  return (
                    <div key={d.id} onClick={() => openDette(d)}
                      ...
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                        <div>
                          <p style={{ margin:0, fontSize:14, fontWeight:700, color:'#1A141A' }}>{d.nom}</p>
                          <p style={{ margin:'2px 0 0', fontSize:11, color:'#8E5915' }}>{d.description}</p>
                          {d.project && <p style={{ margin:'2px 0 0', fontSize:10, color:'#B8A090' }}>📍 {d.project.name}</p>}
                        </div>
                        <div style={{ textAlign:'right', flexShrink:0 }}>
                          <p style={{ margin:0, fontSize:15, fontWeight:800, fontFamily:'monospace', color:'#1A141A' }}>{formatCurrency(Number(d.montant))}</p>
                          <span style={{ fontSize:9, fontWeight:700, padding:'2px 7px', borderRadius:4, background:cfg.bg, color:cfg.color, border:`1px solid ${cfg.border}` }}>
                            {cfg.label}
                          </span>
                        </div>
                      </div>
                      ...
                      <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, color:'#8E5915' }}>
                        <span>Paye : <strong style={{ color:'#10B981' }}>{formatCurrency(Number(d.montant_paye))}</strong></span>
                      ...
                      <div style={{ display:'flex', justifyContent:'flex-end', gap:6, marginTop:8 }}>
                        <button onClick={e => { e.stopPropagation(); openEditDette(d); }}
                        ...
                        <button onClick={e => { e.stopPropagation(); setDeleteDetteTarget(d); }}
                          style={{ padding:'4px 10px', borderRadius:6, border:'1px solid #FECACA', background:'#FFF5F5', color:'#DC2626', fontSize:10, fontWeight:600, cursor:'pointer' }}>
                          <Trash2 size={10}/>
                        </button>
                      </div>
                    </div>
            ...
            <div style={card}>
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                {[
                  ...
                  { label:'Paye',   value:formatCurrency(Number(selectedDette.montant_paye)), color:'#10B981' },
                ...
                <form onSubmit={handlePayDette} style={{ background:'#FFFDF5', border:'1.5px solid #F4B315', borderRadius:10, padding:14, marginBottom:16 }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:10 }}>
                    <div>
                      ...
                      <input type="number" required value={payDetteForm.montant} onChange={e=>setPayDetteForm(f=>({...f,montant:e.target.value}))} style={inputStyle}/>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input type="date" value={payDetteForm.date} onChange={e=>setPayDetteForm(f=>({...f,date:e.target.value}))} style={inputStyle}/>
                    </div>
                    <div>
                      <label style={labelStyle}>Notes</label>
                      <input value={payDetteForm.notes} onChange={e=>setPayDetteForm(f=>({...f,notes:e.target.value}))} placeholder="Optionnel" style={inputStyle}/>
                    </div>
                  </div>
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    ...
                    <button type="submit" disabled={savingPayDette} style={{ ...btnPrimary, flex:2, padding:'8px 0', opacity:savingPayDette?0.7:1 }}>
                      {savingPayDette ? 'Enregistrement...' : 'Confirmer'}
                    </button>
                  </div>
                </form>
              )}
              ...
              {(selectedDette.paiements||[]).length===0 ? (
                <p style={{ textAlign:'center', padding:'20px 0', color:'#8E5915', fontSize:12 }}>Aucun paiement enregistre</p>
              ) : (
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {[...(selectedDette.paiements||[])].sort((a:any,b:any)=>new Date(b.date).getTime()-new Date(a.date).getTime()).map((p:any) => (
                    <div key={p.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 12px', borderRadius:8, background:'#FFFDF7', border:'1px solid #F5E6D3' }}>
                      ...
                      <button onClick={() => handleDeletePaiementDette(p.id)}
        ...
        <div style={{ position:'fixed', inset:0, zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          <div style={{ position:'relative', zIndex:10, background:'white', borderRadius:16, width:'100%', maxWidth:560, margin:'0 16px', boxShadow:'0 20px 60px rgba(0,0,0,0.3)', maxHeight:'80vh', display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'18px 24px', borderBottom:'1px solid #F5E6D3', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <h2 style={{ margin:0, fontSize:16, fontWeight:700, color:'#1A141A' }}>Justificatifs scanes</h2>
            ...
            <form onSubmit={handleSave} style={{ padding:24 }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
                <div style={{ gridColumn:'1/-1' }}>
                  ...
                  <input required value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder="Ex: Achat ciment 50 sacs" style={inputStyle}/>
                </div>
                <div>
                  <label style={labelStyle}>Montant (MAD) *</label>
                  <input required type="number" step="0.01" min="0" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} placeholder="0.00" style={{...inputStyle,fontFamily:'monospace'}}/>
                </div>
                <div>
                ...
                <div>
                  ...
                  <select required value={form.payment_method} onChange={e=>setForm({...form,payment_method:e.target.value})} style={inputStyle}>
                    <option value="ESPECES">Especes</option>
                    <option value="VIREMENT">Virement</option>
                    <option value="CHEQUE">Cheque</option>
                  </select>
                ...
                <div style={{ gridColumn:'1/-1' }}>
                  <label style={labelStyle}>Notes</label>
                  <textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Informations complementaires..." rows={2} style={{...inputStyle,resize:'none'}}/>
                </div>
                <div style={{ gridColumn:'1/-1' }}>
                  ...
                  {!form.receipt_url && !receiptPreview ? (
                    ...
                    <div style={{ border:'1.5px solid #E8D4B0', borderRadius:10, padding:12, background:'#FFFDF7', display:'flex', alignItems:'center', gap:12 }}>
                      {receiptPreview && !receiptIsPdf ? (
                        <img src={receiptPreview} alt="scan" style={{ width:60, height:60, objectFit:'cover', borderRadius:6, border:'1px solid #E8D4B0', cursor:'pointer' }} onClick={() => setViewScan(receiptPreview)}/>
                      ) : (
                        <div style={{ width:60, height:60, borderRadius:6, border:'1px solid #E8D4B0', background:'#FFF3E0', display:'flex', alignItems:'center', justifyContent:'center' }}>
                          <FileText size={24} style={{ color:'#E59312' }}/>
                        </div>
                      )}
                      <div style={{ flex:1 }}>
                        <p style={{ margin:0, fontSize:12, fontWeight:600, color:'#1A141A' }}>{uploadingReceipt ? 'Telechargement...' : receiptIsPdf ? 'PDF joint' : 'Photo jointe'}</p>
                        <p style={{ margin:'2px 0 0', fontSize:11, color:'#8E5915' }}>{form.receipt_url ? 'Scan enregistre' : 'En cours...'}</p>
                      </div>
          ...
          <div style={{ position:'relative', zIndex:10, background:'white', borderRadius:16, width:'100%', maxWidth:480, margin:'0 16px', boxShadow:'0 20px 60px rgba(0,0,0,0.25)' }}>
            ...
            <form onSubmit={handleSaveDette} style={{ padding:24 }}>
              ...
              <div style={{ display:'flex', justifyContent:'flex-end', gap:10, marginTop:16, paddingTop:16, borderTop:'1px solid #F5E6D3' }}>
                ...
                <button type="submit" disabled={savingDette} style={{ ...btnPrimary, opacity:savingDette?0.7:1 }}>
                  {savingDette ? 'Enregistrement...' : (editDette ? 'Enregistrer' : 'Créer')}
                </button>
              </div>
            </form>
          </div>
