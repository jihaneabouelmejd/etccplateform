...
export default function MesFacturesPage() {
  ...
  const fetchFactures = () => {
    depensesApi.findMine()
      .then(r => {
        ...
        setFactures(all.filter((d: any) => d.description?.startsWith('[FAC-ACHAT]')));
      })
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      await depensesApi.create({
        description: `[FAC-ACHAT] ${file.name}`,
        category:    'AUTRE',
        amount:      1,
        receipt_url: url,
      });
      setSuccess(t('myfac.success'));
  ...
  const statusCfg: Record<string, { label: string; bg: string; color: string }> = {
    ...
    APPROVED: { label: t('myfac.approved'), bg: '#E8F5E9', color: '#2E7D32' },
    REJECTED: { label: t('monbl.status_r'), bg: '#FFEBEE', color: '#D32F2F' },
  };

  const totalImported = factures.length;
  const totalApproved = factures.filter(f => f.status === 'APPROVED').length;
  const totalPending  = factures.filter(f => f.status === 'PENDING').length;
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto' }} dir={dir}>
      ...
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1A141A', margin: 0 }}>🧾 {t('myfac.title')}</h1>
        <p style={{ margin: '6px 0 0', fontSize: 13, color: '#8E5915' }}>{t('myfac.subtitle')}</p>
      </div>
      ...
      >
        {uploading ? (
          ...
          <div>
            ...
            <p style={{ margin: '0 0 6px', fontSize: 15, fontWeight: 800, color: '#1A141A' }}>{t('myfac.drop')}</p>
            <p style={{ margin: '0 0 16px', fontSize: 13, color: '#8E5915' }}>{t('myfac.or_click')}</p>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 20px', borderRadius: 8, background: 'linear-gradient(135deg,#F5C842,#D4A017)', color: '#1A141A', fontSize: 13, fontWeight: 700 }}>
              <Upload size={15} /> {t('myfac.btn')}
            </div>
          </div>
        )}
      </div>
      ...
      {factures.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
          {[
            { label: t('myfac.total'),   value: totalImported, accent: '#F5C842' },
            { label: t('myfac.pending'), value: totalPending,  accent: '#D4A017' },
            { label: t('myfac.approved'),value: totalApproved, accent: '#2E7D32' },
          ].map(k => (
            <div key={k.label} style={{ background: 'white', borderRadius: 10, padding: '14px 16px', borderTop: `3px solid ${k.accent}`, boxShadow: '0 2px 6px rgba(142,89,21,0.07)' }}>
              <p style={{ margin: '0 0 4px', fontSize: 9, fontWeight: 700, color: '#8E5915', textTransform: 'uppercase', letterSpacing: 0.8 }}>{k.label}</p>
              <p style={{ margin: 0, fontSize: 24, fontWeight: 900, color: '#1A141A', fontFamily: 'monospace' }}>{k.value}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {factures.map((d: any) => {
            ...
            const fileName = d.description?.replace('[FAC-ACHAT] ', '') || 'Document';
              ...
              <div key={d.id} style={{ background: 'white', border: '1px solid #EDDEC1', borderRadius: 12, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexDirection: dir === 'rtl' ? 'row-reverse' : 'row' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexDirection: dir === 'rtl' ? 'row-reverse' : 'row' }}>
                  ...
                  <div style={{ textAlign: dir === 'rtl' ? 'right' : 'left' }}>
                    ...
                    <div style={{ fontSize: 11, color: '#8E5915', marginTop: 2 }}>{t('myfac.imported_on')} {date}</div>
                  </div>
                </div>
