...
const menuItems = [
  ...
  { href: '/dashboard-gerant',     key: 'menu.dashboard_gerant', emoji: '📋', roles: ['ADMIN', 'GERANT'] },
  ...
  { href: '/veille',               key: 'menu.veille',         emoji: '🔎', roles: ['ADMIN', 'GERANT', 'EMPLOYE'], moduleKey: 'veille' },
...
function DashboardInner({ children }: { children: React.ReactNode }) {
  ...
  // NB: doit rester AVANT le "if (!isReady) return" pour ne pas violer les
  // règles des hooks (nombre de hooks constant entre les renders).
  const guardRole = (user?.role || '').toUpperCase();
  const guardAllowedModules: string[] = Array.isArray(user?.allowed_modules) ? user.allowed_modules : [];
  const moduleGuardActive = guardRole === 'EMPLOYE' && guardAllowedModules.length > 0;
  const isDashboardRoute = pathname === '/dashboard' || pathname.startsWith('/dashboard/');
  const currentPageForGuard = menuItems.find(item =>
    pathname === item.href || pathname.startsWith(item.href + '/')
  );
  const pageAllowed = !moduleGuardActive || isDashboardRoute ||
    !!(currentPageForGuard && (currentPageForGuard as any).moduleKey && guardAllowedModules.includes((currentPageForGuard as any).moduleKey));

  useEffect(() => {
    if (isReady && moduleGuardActive && !pageAllowed) {
      window.location.href = '/dashboard';
    }
  }, [isReady, moduleGuardActive, pageAllowed, pathname]);
